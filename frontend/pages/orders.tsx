      ...
      <div className={`min-h-screen p-8 bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100 transition-all duration-700 ${fadeIn ? "opacity-100" : "opacity-0"}`}>
        <h1 className="text-2xl font-bold mb-4 flex items-center gap-2">
          ...
          <span className="relative group">
            ...
            <span className="absolute left-6 top-0 bg-black text-white text-xs rounded px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">Gestión de pedidos</span>
          </span>
        </h1>
        <table className="w-full border-collapse bg-white rounded-lg shadow transition-all duration-700">
          ...
          <tbody>
            {orders.map((o) => (
              <tr key={o.id} className="hover:bg-blue-50 transition-colors group">
                <td>{o.id}</td>
